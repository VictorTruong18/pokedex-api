...
export const checkAuthorization = async (req, res, next) => {
  ...
  try {
    ...
    res.locals.requestor = await jwt.verify(
      bearerToken[1],
    ...
    return res
      .status(500)
      .send({ error: "Error with the JWT verification process" });
  }
  next();
};
...
export const isDresseurAccount = async (req, res, next) => {
  ...
  if (id != res.locals.requestor.id && !res.locals.requestor.isAdmin) {
    return res
      .status(403)
      .send({ error: "You don't have the privilege to do this action" });
  }
...
export const isDresseurPokemon = async (req, res, next) => {
  try {
    ...
    if (
      dresseurId != res.locals.requestor.id &&
      !res.locals.requestor.isAdmin
    ) {
      return res.status(403).send({
        error:
          "You don't have the privilege to modify a Pokemon that doesn't belong to you",
      });
    }
  } catch (err) {
    return res.status(500).send({ error: err });
  ...
  try {
    if (!res.locals.requestor.isAdmin) {
      if (dresseurId) {
        return res
          .status(403)
          .send({ error: "As a USER you cant modify the owner of a pokemon" });
      }
    } else {
      // is the dresser existant
      if (dresseurId) {
        ...
        if (!dresseur) {
          return res
            .status(404)
            .send(
              "The owner you want to attribute this pokemon does not exist"
            );
        }
      }
    }
...
export const isAllowedToCreateDresseur = async (req, res, next) => {
  ...
  if (!authorization) {
    ...
    try {
      ...
      const parsedDresseur = await jwt.verify(
        bearerToken[1],
        "ServerInternalPrivateKey"
      );
      ...
      if (Dresseur.hasRoles(dresseur, "ADMIN")) {
        ...
        return res
          .status(403)
          .send({ error: "You need to be ADMIN to create another Dresseur" });
      }
    } catch (err) {
      if (err instanceof jwt.TokenExpiredError) {
        return res.status(401).send({ error: "Token expired" });
