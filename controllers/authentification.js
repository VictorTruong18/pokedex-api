...
export const authorizationCodeCheck = async (req, res) => {
  ...
  return res.redirect(
    `${queryParams.redirect_uri}?authorization_code=${authorizationCode}`
  );
};
...
export const authorizationCodeDisplay = async (req, res) => {
  ...
  return res
    .status(200)
    .send(
      "Here is your authorization code for login : " +
        queryParams.authorization_code
    );
};
...
export const generateAccessToken = async (req, res) => {
  ...
  if (
    queryParams.client_id !== acceptedClientId ||
  ...
  return res
    .status(200)
    .send({ accessToken, tokenType: "Bearer", expiresIn: "30m" });
};
